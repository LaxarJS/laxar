...
This minimizes the _footprint_ of widgets -- great if they are to be used from projects that may already be using one or more integration technologies.
...
These do not benefit as much from MVC frameworks, except maybe from their utility services (such as `$http` provided by AngularJS).
...
Again, we start with a `widget.json` descriptor.
...
Put your template under `default.theme/my-widget.html`, just like you would do for other technologies:
...
Using `axControls.provide( name )`, they can obtain the modules by `name` like in the `controls` section of their widget descriptor.
