...
webpackConfig.entry = glob.sync( specPattern ).reduce( ( acc, path ) => {
...
webpackConfig.output = {
   ...
   filename: '[name]/spec-runner.bundle.js'
};

webpackConfig.plugins = [
   new webpack.SourceMapDevToolPlugin( {
      filename: '[name]/spec-runner.bundle.js.map'
   } ),
...
function JasmineHtmlRunnerWebpackPlugin( optionalOptions ) {
   const options = Object.assign( {}, optionalOptions || {}, {
      ...
      babelPolyfillPath: '../../node_modules/babel-polyfill/dist/polyfill.js'
   } );
   ...
   this.apply = function( compiler ) {
      compiler.plugin('emit', ( compilation, done ) => {
         compilation.chunks.forEach( ( chunk ) => {
            ...
            compilation.assets[ chunk.name + '/spec-runner.html' ] = {
               source: () => source,
               size: () => source.length
            };
         } );
      ...
      return `
         <html>
           <head>
              ...
              <link type="text/css" rel="stylesheet" href="${ctx.jasminePath}/jasmine.css"></link>
              ...
              ${ctx.polyfills.map( path => `<script type="text/javascript" src="${path}"></script>`).join('\n')}
           ...
           <body>
             <script src="http://${ctx.host}:${ctx.port}/webpack-dev-server.js"></script>
             <script src="spec-runner.bundle.js"></script>
           </body>
         </html>
      `;
   }
