 ...
 *    an i18n instance
 */
export function create( context, configuration, optionalOptions = {} ) {

   const { features, eventBus, widget } = context;
   ...
   const unsubscribe = eventBus.subscribe( 'didChangeLocale', handleLocaleChange );

   const fallbackLocale = configuration.get( 'i18n.locales.default', fallback );
   const defaultLocale = path( features, 'i18n.locale', fallbackLocale );
   ...
   function forFeature( featurePath ) {
      ...
      assert( locale ).hasType( String ).isNotNull(
         `axI18n: missing feature-configuration '${featurePath}.locale' (widget: ${widget.id})`
      );

      const api = forLocale( locale, featurePath );
   ...
   function forLocale( locale, trackingProperty ) {
      assert( trackingProperty ).hasType( String ).isNotNull();
       ...
       * @memberof AxI18nHandler
       */
      function track( property = trackingProperty ) {
