...
export function create( browser, configuration, Navigo = NavigoImplementation ) {
   ...
   const queryParametersByPattern = {};
    ...
    *
    * @param {Object.<String, Object>} routeMap
    *    a map of routing patterns in Navigo syntax to the corresponding route handlers. Each route handler
    *    has up to two fields:
    *    - `handler` (Function, required): the route implementation which will be invoked with parameters
    *      when its route is matched by the browser location. The handler will receive the decoded parameter
    *      values for their pattern and (if configured) from the query string, as a map from string parameter
    *      name to string value.
    *    - `additionalParameters` (Array<String>, optional): Navigo does not support query parameters as part
    *      of pattern definitions. By default, all query parameters from the location are added to the handler
    *      parameters, and all parameters are encoded as query-parameters during reverse-routing. If the
    *      `additionalParameters` are specified however, only the parameters from the list will be read
    *      from the query string, or added when generating URLs for a route.
    *
   ...
   function registerRoutes( routeMap, fallbackHandler ) {
      let emptyHashRoute = null;
      if( useHash && '/' in routeMap ) {
         emptyHashRoute = routeMap[ '/' ];
         delete routeMap[ '/' ];
      }

      const navigoRoutes = {};
      Object.keys( routeMap ).forEach( pattern => {
         const { handler, additionalParameters } = routeMap[ pattern ];
         queryParametersByPattern[ pattern ] = tabulate( _ => true, additionalParameters );
         navigoRoutes[ pattern ] = ( params, querystring ) => {
            const frag = collectFragment();
            handler( collectParameters( pattern, params, querystring, frag ), frag );
         };
      } );
         ...
         .on( '*', () => {
            if( emptyHashRoute ) {
               ...
               if( [ '', hash, `${hash}/` ].indexOf( hash ) !== -1 ) {
                  const frag = collectFragment();
                  emptyHashRoute.handler( collectParameters( '*', {}, querystring, frag ) );
                  return;
               }
            }
   ...
   function collectParameters( pattern, params, querystring, frag ) {
      const routingUri = browser.location().hash.slice( hash.length );

      const parameters = {};
      if( queryEnabled && querystring && querystring.length ) {
         const queryParameters = queryParametersByPattern[ pattern ];
         console.log( '[TODO: DELETE ME] queryParameters', queryParametersByPattern[ pattern ] );
         withoutNestedFragment( querystring ).split( '&' )
            .map( _ => _.split( '=' ).map( decodeURIComponent ) )
            .filter( ([ key ]) => !queryParameters || queryParameters[ key ] )
            .forEach( ([ key, value ]) => {
