   ...
   function registerRoutes( routeMap, fallbackHandler ) {
      ...
      const preparedRoutes = Object.keys( routeMap ).reduce( ( routes, pattern ) => ({
         ...
         [ pattern ]: ( params, querystring ) => {
            routeMap[ pattern ]( collectParameters( params, querystring ), fragment() );
         }
      }), {} );
         ...
         .on( '*', querystring => {
            if( emptyHashRouteHandler ) {
               if( [ '', hash, `${hash}/` ].indexOf( browser.location().hash ) !== -1 ) {
                  emptyHashRouteHandler( collectParameters( {}, querystring ), fragment() );
                  return;
               }
            }
      ...
      function fragment() {
         ...
         const pathFragment = location.hash.slice( hash.length );
         const start = pathFragment.indexOf( '#' );
         return start === -1 ? null : decodeURIComponent( pathFragment.slice( start + 1 ) );
      }
