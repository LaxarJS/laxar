...
export function create( browser, configuration, Navigo = NavigoImplementation ) {
   ...
   const browserDocumentUrl = browser.location().origin + browser.location().pathname;
   ...
   function constructAbsoluteUrl( patterns, parameters, fragment = null ) {
      ...
      let url = router.link( ( useHash ? hash : '' ) + path );
      if( !useHash ) {
         return url;
      }
      ...
      url = endsWith( url, brokenSuffix ) ?
         `${url.slice( 0, url.lastIndexOf( brokenSuffix ) )}/${hash}${path}` :
         url;

      // another problem with Navigo (?): if the initially visited URL ends with a slash,
      // that slash is missing from generated URLs:
      const routingSuffix = `${hash}${path}`;
      url = url.lastIndexOf( `${routingSuffix}` ) < browserDocumentUrl.length ?
         `${url.slice( 0, url.lastIndexOf( `${routingSuffix}` ) )}/${routingSuffix}` :
         url;
