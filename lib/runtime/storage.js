 ...
 * an arbitrary and a configured namespace to prevent naming clashes with other (LaxarJS) web applications
 * running on the same host and port. All {@link StorageApi} accessor methods should then be called without
 * any namespace since adding and removing it, is done automatically.
 *
 * In widgets and activities, it is recommended to access storage through the `axStorage` injection`.
 *
 * When importing `laxar`, a storage instance is also available as `laxar.storage` for backwards
 * compatibility.
 *
 * @module storage
 */
...
function getOrFakeBackend( browser, webStorageName ) {
   ...
   if( store.setItem && store.getItem && store.removeItem ) {
      try {
         const testKey = 'ax.storage.testItem';
...
function generateUniquePrefix( configuration ) {
   const prefix = configuration.get( 'storagePrefix' );
   if( prefix ) {
      return prefix;
   }

   const str = configuration.get( 'name', '' );
   let res = 0;
   // TODO (#319) simplify!
...
export function create( configuration, browser, localStorageBackend, sessionStorageBackend ) {
   ...
   const prefix = 'ax.' + generateUniquePrefix( configuration ) + '.';
