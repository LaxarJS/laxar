 ...
 *
 * This router allows to register flow patterns in page.js syntax so that their handler is activated when the
 * corresponding URL is entered in the browser. While that alone does not add much to the functionality built
 * into page.js, this router also allows to construct URLs based on a pattern and corresponding substitution
 * parameters. Finally, users can trigger navigation directly.
 *
 * Note that the router supports various configuration options:
 *
 *  - `router.pagejs`: configuration object that is directly passed to page.js (such as `click`, `popstate`,
 *    `dispatch`, `hashbang`). The application is responsible for specifying the required options, as LaxarJS
 *    does not touch the page.js defaults otherwise. Consult the page.js documentation for more information
 *  - `router.query.enabled`: if `true`, query parameters are automatically transformed into additional place
 *    parameters and vice versa. The default is `false`
 *  - `router.base`: The base path under which to perform routing. If omitted, the document base href is used
 *
 *
...
export function create( pagejs, browser, configuration ) {
   ...
   /**
    * Creates and returns a new pagejs router instance from its dependencies.
    *
    * @name PagejsRouter
    * @constructor
    */
