...
export function createConsoleChannel( browser ) {
   ...
   return function log( messageObject ) {
      ...
      console.log( `${level}: ${message} (@ ${file}:${line})`, ...interpolations );
   };
...
const CONSOLE_COMPATIBLE_FORMAT_MATCHER = /^[%](\.\d+f|[dios])$/;
   ...
   while( pos < text.length ) {
      ...
      switch( character ) {
         ...
         case '[': {
            ...
            const formatSpecifier = replacementString
               .split( ':' )
               .slice( 1 )
               .filter( _ => CONSOLE_COMPATIBLE_FORMAT_MATCHER.test( _ ) )
               .pop();
