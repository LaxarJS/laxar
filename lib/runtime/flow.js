...
const module = ng.module( 'axFlow', [ 'ngRoute' ] );
...
module.run( [
   ...
   function( $route, configuration, fileResourceProvider ) {
      ...
      loadFlow( path.normalize( paths.FLOW_JSON ) ).then( () => $route.reload() );
   } ]
);
...
let activeTarget_ = TARGET_SELF;
let activePlace_ = null;
let activeParameters_ = {};

let places_;
let previousNavigateRequestSubscription_;
let navigationInProgress_ = false;
let navigationTimer_;

const eventOptions = { sender: 'AxFlowController' };
const subscriberOptions = { subscriber: 'AxFlowController' };
...
module.controller( 'AxFlowController', [
   ...
   function FlowController( $window, $location, $routeParams, place, eventBus, flowService, pageService ) {
      ...
      const navigateEvent = { target: activeTarget_ };
      const didNavigateEvent =  object.options( { data: {}, place: place.id }, navigateEvent );
      ...
      function handleNavigateRequest( event, meta ) {
         ...
         const placeName = placeNameForNavigationTarget( activeTarget_, place );
         const newPlace = places_[ placeName ];
         ...
         const newPath = flowService.constructPath( event.target, event.data );
...
module.factory( 'axFlowService', [ '$location', function( $location ) {
   ...
   const flowService = {
      ...
      constructPath: function( targetOrPlace, optionalParameters ) {
         const newParameters = object.options( optionalParameters, activeParameters_ || {} );
         const placeName = placeNameForNavigationTarget( targetOrPlace, activePlace_ );
         const place = places_[ placeName ];
      ...
      constructAbsoluteUrl: function( targetOrPlace, optionalParameters ) {
         const absUrl = $location.absUrl().split( '#' )[0];
...
function placeNameForNavigationTarget( targetOrPlaceName, activePlace ) {
   const placeName = object.path( activePlace, 'targets.' + targetOrPlaceName, targetOrPlaceName );
...
function routeByEntryPoint( possibleEntryPoints ) {
   const entryPoint = entryPoint_ || { target: 'default', parameters: {} };

   const placeName = possibleEntryPoints[ entryPoint.target ];
   if( placeName ) {
      const targetPlace = places_[ placeName ];
...
function validateFlowJson( flowJson ) {
   const result = jsonValidator.create( flowSchema ).validate( flowJson );
