...
ThemeManager.prototype.urlProvider = function( artifactPathPattern, themePathPattern, fallbackPathPatterns ) {
   ...
   return {
      provide( fileNames ) {
         ...
         const themeDirectory = self.theme + '.theme';
         ...
         ( fallbackPathPatterns || [] ).forEach( pattern => {
            ...
            if( self.theme !== 'default' || pattern.indexOf( '[theme]' ) === -1 ) {
...
function findExistingPath( self, searchPrefixes, fileName ) {
   ...
   return self.fileResourceProvider.isAvailable( path.join( searchPrefixes[ 0 ], fileName ) )
      .then( available =>
