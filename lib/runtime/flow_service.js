...
export function create( fileResourceProvider, eventBus, configuration, browser, pageService, router=pageRouter ) {
   ...
   function createFlowController() {
      ...
      function placeNameForNavigationTarget( targetOrPlaceName, place ) {
         ...
         log.error( `Unknown target or place "${targetOrPlaceName}". Current place: "${place.id}"` );
      }
      ...
      return fileResourceProvider.provide( flowFile )
         ...
         .then( places => {
            ...
            function routeByEntryPoint( possibleEntryPoints ) {
               const entryPoint = configuration.get( 'flow.entryPoint', { target: 'default', parameters: {} } );
