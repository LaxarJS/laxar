...
export function create(
   ...
   const {
      base = fallbackBase( routerConfiguration ),
      ...pageOptions
   } = routerConfiguration;
   ...
   const api = {
      ...
      /**
       * Alias for {@link #absoluteUrl}, for backward-compatibility.
       *
       * @type {Function}
       * @name constructAbsoluteUrl
       * @deprecated Use {@link #absoluteUrl} instead
       */
      constructAbsoluteUrl: absoluteUrl
   };
    ...
    */
   function absoluteUrl( targetOrPlace, optionalParameters ) {
      const routingPath = constructPath( targetOrPlace, optionalParameters );
      return useHashbang ? `${base}#!${routingPath}` : `${base}${routingPath}`;
   }
...
function encodePlaceParameter( value ) {
   return encodeURIComponent( value == null ? '_' : value );
}
