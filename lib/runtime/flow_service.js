 ...
 */
import pageRouter from './router_page';
...
export function create(
   ...
   function constructPath( targetOrPlace, optionalParameters ) {
      ...
      const segments = placeName ? [ placeName ] : [];
      place.expectedParameters.forEach( _ => {
         segments.push( encodeSegment( newParameters[ _ ] ) );
         delete newParameters[ _ ];
      } );
      // normalize path:
      while( segments[ segments.length - 1 ] === '_' ) {
         segments.pop();
      }
...
function processPlaceParameters( places ) {
   ...
   object.forEach( places, ( place, placeName ) => {
      ...
      while( ( matches = ROUTE_PARAMS_MATCHER.exec( placeName ) ) ) {
         ...
         place.expectedParameters.push( matches[ 2 ] );
