...
export function create( eventBus, heartbeat, pageLoader, layoutLoader, widgetLoader, themeManager, localeManager, visibilityManager ) {
   ...
   function createPageController( pageElement ) {
      ...
      function setupPage( pageName ) {
         ...
         const pagePromise = pageLoader.load( pageName )
            .then( page => {
               areaHelper = createAreaHelper( q, page );
               visibilityManager.setAreaHelper( areaHelper );

               layoutLoader.load( page.layout ).then( layoutInfo => layoutDeferred.resolve( layoutInfo ) );
         ...
         const widgetsInitialized = pagePromise.then( () => delay( WIDGET_ATTACH_DELAY_MS ) );
         const layoutReady = layoutDeferred.promise
            .then( result => cleanUpLayout = renderLayout( pageElement, areaHelper, result ) );
