...
describe( 'An artifactProvider', () => {
   ...
   beforeEach( () => {
      ...
      browserMock.respondWith( 'ganz/egal/wo', { myMessage: 'Guten Tack!' } );
      configurationMock = createConfigurationMock( { theme: 'western' } );
   ...
   describe( 'configured with a theme that does not exist', () => {
      ...
      beforeEach( () => {
         configurationMock = createConfigurationMock( { theme: 'missing' } );
         artifactProvider = createArtifactProvider( artifactsMock, browserMock, configurationMock, logMock );
      } );
   ...
   describe( 'when asked for a theme', () => {
      ...
      it( 'selects the theme based on the configuration', done => {
         expect( configurationMock.get ).toHaveBeenCalledWith( 'theme', 'default' );
      ...
      it( 'provides a URL for the theme\'s main CSS file', done => {
         themeProvider.assetUrlForTheme( 'css/theme.css' )
            .then( url => {
   ...
   describe( 'when asked for a widget', () => {
      ...
      it( 'provides access to custom assets referenced by URL', done => {
         widgetProvider.asset( 'messages.de.json' )
            .then( result => {
               expect( browserMock.fetch ).toHaveBeenCalledWith( 'ganz/egal/wo' );
               expect( JSON.parse( result ) ).toEqual( { myMessage: 'Guten Tack!' } );
            } )
            .then( done, done.fail );
      } );
