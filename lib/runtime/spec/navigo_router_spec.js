...
describe( 'A navigo router', () => {
   ...
   beforeEach( () => {
      ...
      locationMock = createLocationMock( configurationData[ 'router.navigo.hash' ] || '#' );
   ...
   describe( 'without route map, intended for constructing URLs', () => {
      ...
      describe( 'with the hash-based routing configuration', () => {
         ...
         it( 'calls navigo.link with prepended hash (#440)', () => {
            router.constructAbsoluteUrl( [ '/prefixTwo' ], {} );
            const [ path ] = navigoMock.instance.link.calls.first().args;
            expect( path ).toMatch( /^#\// );
         } );

         /////////////////////////////////////////////////////////////////////////////////////////////////////

         it( 'encodes an additional fragment using a nested hash (#464)', () => {
            expect(
               router.constructAbsoluteUrl( [ '/:x/y' ], { x: 'XX', z: 'ZZ' }, 'frag' )
            ).toEqual( 'https://server:4711/path#/XX/y#frag' );
         } );

         /////////////////////////////////////////////////////////////////////////////////////////////////////

         it( 'URI-encodes an additional nested hash fragment (#464)', () => {
            expect(
               router.constructAbsoluteUrl( [ '/:x/y' ], { x: 'XX', z: 'ZZ' }, 'fr#ag' )
            ).toEqual( 'https://server:4711/path#/XX/y#fr%23ag' );
         } );
      ...
      describe( 'configured with custom hash when using hash-based URLs', () => {

         overrideConfig( 'router.navigo.useHash', true );
         overrideConfig( 'router.navigo.hash', '#!' );

         /////////////////////////////////////////////////////////////////////////////////////////////////////

         it( 'creates absolute URLs with that custom hash', () => {
            expect(
               router.constructAbsoluteUrl( [ '/prefixTwo' ], {} )
            ).toEqual( 'https://server:4711/path#!/prefixTwo' );
         ...
         it( 'to construct URLs, calls navigo.link with prepended custom hash (#440)', () => {
            router.constructAbsoluteUrl( [ '/prefixTwo' ], {} );
            const [ path ] = navigoMock.instance.link.calls.first().args;
            expect( path ).toMatch( /^#!\// );
         } );
      ...
      describe( 'configured to work without hash/hashbang in URLs', () => {
         ...
         it( 'to construct URLs, calls navigo.link without prepended hash (#440)', () => {
            router.constructAbsoluteUrl( [ '/prefixTwo' ], {} );
            const [ path ] = navigoMock.instance.link.calls.first().args;
            expect( path ).toMatch( /^\// );
         } );
         ...
         it( 'allows to construct URLs with an additional fragment', () => {
            let url;

            url = router.constructAbsoluteUrl( [ '/prefixTwo' ], null, 'hey' );
            expect( url ).toEqual( 'https://server:4711/the-base/prefixTwo#hey' );

            url = router.constructAbsoluteUrl(
               [ '/prefixOne/:param', '/prefixTwo' ],
               { param: 'vanilla-ice' },
               'ho'
            );
            expect( url ).toEqual( 'https://server:4711/the-base/prefixOne/vanilla-ice#ho' );
         } );

         /////////////////////////////////////////////////////////////////////////////////////////////////////

         it( 'URL-encodes any additional fragment', () => {
            const url = router.constructAbsoluteUrl( [ '/prefixTwo' ], null, '#BAM!?#' );
            expect( url ).toEqual( 'https://server:4711/the-base/prefixTwo#%23BAM!%3F%23' );
         } );
   ...
   describe( 'initialized with a map of routes', () => {
      ...
      describe( 'while using hash-based URLs', () => {
         ...
         describe( 'when navigo handles a route change with parameters and fragment', () => {

            beforeEach( () => {
               locationMock.hash = '#/prefixOne/a%23keep/b#use#this';
               navigoMock.triggerRoute( '/prefixOne/:paramA/:paramB', {
                  paramA: 'a#keep', paramB: 'b#use#this'
               } );
            } );

            //////////////////////////////////////////////////////////////////////////////////////////////////

            it( 'passes the fragment to the matching handler', () => {
               const args = spyOneBinary.calls.argsFor( 0 );
               expect( args[ 1 ] ).toEqual( jasmine.any( String ) );
            } );

            //////////////////////////////////////////////////////////////////////////////////////////////////

            it( 'URL-decodes the fragment before passing it to the handler', () => {
               const args = spyOneBinary.calls.argsFor( 0 );
               expect( args[ 1 ] ).toEqual( 'use#this' );
            } );

            //////////////////////////////////////////////////////////////////////////////////////////////////

            it( 'strips the nested fragment from its containing place parameter', () => {
               const args = spyOneBinary.calls.argsFor( 0 );
               expect( args[ 0 ] ).toEqual( { paramA: 'a#keep', paramB: 'b' } );
            } );
      ...
      describe( 'when navigo handles a route change with parameters', () => {
         ...
         it( 'runs the correct handler with the respective parameters', () => {
            expect( spyOneUnary ).toHaveBeenCalledWith( { p: 'x' }, null );
         } );
      ...
      describe( 'when navigo handles a route change with parameters and fragment', () => {
         ...
         beforeEach( () => {
            locationMock.hash = '#x%3D34%26y%3D12';
            navigoMock.triggerRoute( '/prefixOne/:p', { p: 'x' } );
         } );
         ...
         it( 'passes the fragment to the matching handler', () => {
            const args = spyOneUnary.calls.argsFor( 0 );
            expect( args[ 1 ] ).toEqual( jasmine.any( String ) );
         } );
         ...
         it( 'URL-decodes the fragment before passing it to the handler', () => {
            const args = spyOneUnary.calls.argsFor( 0 );
            expect( args[ 1 ] ).toEqual( 'x=34&y=12' );
         } );
      ...
      describe( 'when navigo handles a route change with underscore for segment parameters', () => {
         ...
         it( 'transforms the missing parameters to null', () => {
            expect( spyOneBinary ).toHaveBeenCalledWith( { paramA: null, paramB: 'X' }, null );
         } );
      ...
      describe( 'when navigo handles a route change, passing encoded slashes or underscores', () => {
         ...
         it( 'reverts the double-encoding', () => {
            expect( spyOneBinary ).toHaveBeenCalledWith( { paramA: '/', paramB: '_' }, null );
         } );
      ...
      describe( 'when navigo handles a route change, passing query parameters', () => {
         ...
         beforeEach( () => {
            navigoMock.triggerRoute(
               ...
               { paramA: '_', paramB: 'XYZ' },
               'paramA=1000&paramC=5000&paramD'
            );
   ...
   function createLocationMock( hash ) {
      return {
         protocol: 'https',
         hostname: 'server',
         port: 4711,
         pathname: '/path',
         hash: `${hash}/editor`,
         href: `https://server:4711/path${hash}/editor`
      };
   }
