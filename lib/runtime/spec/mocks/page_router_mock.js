...
export function create() {
   ...
   page.start = jasmine.createSpy( 'pageRouterMock.start' ).and.callFake( () => {
      started = true;
   ...
   page.show = jasmine.createSpy( 'pageRouterMock.show' ).and.callFake( url => {
      const reaction = showReactions[ url ] || showReactions[ '*' ];
      assert.state( !!reaction, `page.show was called without matching expectation (url: ${url})` );
      reaction( url );
      delete showReactions[ url ];
      delete showReactions[ '*' ];
   } );
   ...
   const mock = {
      ...
      awaitStart() {
         return started ?
            Promise.resolve() :
            new Promise( _ => { resolveStartPromise = _; } );
      },
      awaitShow( url ) {
         return new Promise( _ => { showReactions[ url || '*' ] = _; } );
      }
   };
   ...
   function normalizePageInvocation( a, b ) {
      ...
      assert.codeIsUnreachable( 'page.js invocation not recognized: [0], [1]', a, b );
      return null;
   }
