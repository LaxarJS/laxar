...
describe( 'A flow controller instance', () => {
   ...
   function startAtRoute( routeName, optionalContext = {} ) {
      return flowController.loadFlow( 'mainz' )
   ...
   describe( 'when the flow is loaded', () => {
      ...
      it( 'registers router-handlers for places with the `redirectTo` or `redirectToPath` property', () => {
      ...
      describe( 'and then asked to construct a URL', () => {
         ...
         beforeEach( () => {
            routerMock.router.constructAbsoluteUrl.and.callFake( () => 'http://myserver/path' );
         ...
         describe( 'with an active set of place parameters', () => {

            beforeEach( done => {
               routerMock.triggerRouteHandler( '/evaluation/:dataId/method/:method', { method: 'ABC' } );

               awaitDidNavigate()
                  .then( () => {
                     routerMock.router.constructAbsoluteUrl.calls.reset();
                     flowController.constructAbsoluteUrl( 'evaluation', {
                        dataId: 'XYZ'
                     } );
                     flowController.constructAbsoluteUrl( 'evaluation', {
                        dataId: 'XYZ',
            ...
            it( 'takes into account the currently active place parameters', () => {
               expect( routerMock.router.constructAbsoluteUrl.calls.argsFor( 0 ) ).toEqual( [
                  [ '/evaluation/:dataId/method/:method', '/evaluation/:dataId' ],
                  { method: 'ABC', dataId: 'XYZ' },
                  undefined
               ] );
            } );
            ...
            it( 'overrides the currently active place parameters as needed', () => {
               expect( routerMock.router.constructAbsoluteUrl.calls.argsFor( 1 ) ).toEqual( [
                  [ '/evaluation/:dataId/method/:method', '/evaluation/:dataId' ],
                  { dataId: 'XYZ', method: 'PQR' },
                  undefined
               ] );
            } );
      ...
      describe( 'and the current place is entered by the router (same parameters, new fragment) ', () => {

         beforeEach( done => {
            eventBusMock.publish.calls.reset();
            routerMock.triggerRouteHandler( '/editor/:dataId', {}, 'new-fragment' );
            awaitDidNavigate().then( done, done.fail );
         } );
         ...
         it( 'publishes a corresponding willNavigate event', () => {
            expect( eventBusMock.publish ).toHaveBeenCalledWith( 'willNavigate.editor', {
               target: 'editor',
               place: 'editor',
               data: {},
         ...
         it( 'eventually publishes a didNavigate event', () => {
            expect( eventBusMock.publish ).toHaveBeenCalledWith( 'didNavigate.editor', {
               target: 'editor',
               place: 'editor',
               data: {},
               fragment: 'new-fragment'
      ...
      describe( 'on navigate request to a different target, with fragment specified', () => {
         ...
         beforeEach( done => {
            ...
            routerMock.router.navigateTo.and.callFake( ( path, data, { fragment } ) => {
               routerMock.triggerRouteHandler( path, data, fragment );
            } );
         ...
         it( 'propagates the fragment as part of `willNavigate`', () => {
            expect( eventBusMock.publish ).toHaveBeenCalledWith( 'willNavigate.welcome', {
               target: 'welcome',
               place: 'welcome',
               data: {},
      ...
      describe( 'when navigating to a place with redirectTo', () => {

         beforeEach( () => {
            routerMock.triggerRouteHandler( '/entry', { dataId: 100 } );
         } );

         /////////////////////////////////////////////////////////////////////////////////////////////////////

         it( 'asks the router to actually load the redirectTo place, overwriting the history', () => {
            expect( routerMock.router.navigateTo )
               .toHaveBeenCalledWith( [ '/editor/:dataId' ], { 'dataId': 100 }, { replaceHistory: true } );
         } );
      ...
      describe( 'when navigating to a place with redirectToPath', () => {

         beforeEach( () => {
            routerMock.triggerRouteHandler( '/backdoor' );
         } );

         /////////////////////////////////////////////////////////////////////////////////////////////////////

         it( 'asks the router to actually navigate to the defined path, overwriting the history', () => {
            expect( routerMock.router.navigateToPath )
               .toHaveBeenCalledWith( '/evaluation/123/method/sum', { replaceHistory: true } );
         } );
