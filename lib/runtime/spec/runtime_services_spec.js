...
define( [
   ...
   '../../utilities/object',
   '../../utilities/storage'
], function( runtimeServicesModule, configuration, angularMocks, log, fileResourceProvider, jqueryMock, portalMocks, object, storage ) {
   'use strict';
   ...
   describe( 'The axGlobalEventBus factory', function() {
      ...
      beforeEach( function() {
         ...
         angularMocks.module( runtimeServicesModule.name );

         angularMocks.inject( function( axGlobalEventBus ) {
   ...
   describe( 'The axConfiguration factory', function() {
      ...
      it( 'returns an configuration instance', angularMocks.inject( function( axConfiguration ) {
         expect( axConfiguration.get ).toBeDefined();
      } ) );

      ////////////////////////////////////////////////////////////////////////////////////////////////////////

      it( 'populates the configuration instance with values from a configuration file', angularMocks.inject( function( axConfiguration ) {
      ...
      it( 'lets the configuration return null for non-existing keys', angularMocks.inject( function( axConfiguration ) {
         expect( axConfiguration.get( 'key_three' ) ).toEqual( null );
      } ) );

      ////////////////////////////////////////////////////////////////////////////////////////////////////////

      it( 'lets the configuration return a default value for non-existing keys if given', angularMocks.inject( function( axConfiguration ) {
   ...
   describe( 'The axFileResourceProvider factory', function() {
      ...
      it( 'returns a file resource provider instance', angularMocks.inject( function( axFileResourceProvider ) {
      ...
      describe( 'when there are file listings configured', function() {
         ...
         it( 'sets the given filelistings', angularMocks.inject( function( axFileResourceProvider ) {
   ...
   describe( 'The axCssLoader-factory', function() {
      ...
      it( 'returns a css loader instance', angularMocks.inject( function( axCssLoader ) {
   ...
   describe( 'The axThemeManager-factory', function() {
      ...
      beforeEach( angularMocks.inject( function( $timeout, $rootScope, axGlobalEventBus, axConfiguration ) {
         eventBus_ = axGlobalEventBus;
         spyOn( eventBus_, 'subscribe' ).andCallThrough();
         spyOn( eventBus_, 'publish' ).andCallThrough();

         configuration_ = axConfiguration;
      ...
      beforeEach( angularMocks.inject( function( axThemeManager ) {
   ...
   describe( 'An axLogHttpInterceptor', function() {
      ...
      describe( 'with configured header', function() {
         ...
         it( 'intercepts requests', angularMocks.inject( function( axLogHttpInterceptor ) {
            expect( axLogHttpInterceptor.request ).toEqual( jasmine.any( Function ) );
         } ) );

         it( 'generates the correct header', angularMocks.inject( function( $http ) {
            log.context.addTag( 'MY_TAG', 'test' );
            $httpBackend_.expectGET( '/myService' ).respond( function( method, url, data, headers ) {
               var instanceId = log.context.gatherTags()[ 'INST' ];
               expect( headers[ 'x-0815-tags' ] ).toEqual( '[INST:' + instanceId + '][MY_TAG:test]' );
               return [ 200 ];
            } );
            $http.get( '/myService' );
            $httpBackend_.flush();
      ...
      describe( 'without configured header', function() {
         ...
         it( 'does not intercept requests', angularMocks.inject( function( axLogHttpInterceptor ) {
            expect( axLogHttpInterceptor.request ).toBeUndefined();
         } ) );

         it( 'generates no additional header', angularMocks.inject( function( $http ) {
            $httpBackend_.expectGET( '/myService' ).respond( function( method, url, data, headers ) {
