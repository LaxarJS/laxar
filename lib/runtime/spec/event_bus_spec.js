...
const setup = () => {
   ...
   errorHandler = createLogErrorHandler( logMock );
   nextTick = jasmine.createSpy( 'nextTick' ).and.callFake( f => setTimeout( f, 0 ) );
   eventBus = createEventBus( configurationMock, nextTick, setTimeout, errorHandler );
};
...
describe( 'an event bus can have inspectors', () => {
   ...
   it( 'that are called on every unsubscribe (#20)', () => {
      ...
      const unsubscribe = eventBus.subscribe( 'someEvent', subscriber, { subscriber: 'subscriberX' } );
...
describe( 'An event object', () => {
   ...
   describe( 'where the subscriber doesn\'t request a clone', () => {
      ...
      it( 'shares the delivered event object with other subscribers but not the publisher', done => {
         const event = { data: { x: 12 } };
         eventBus.publish( 'myEvent', event )
            .then( () => {
               expect( data1 ).toEqual( event.data );
               expect( data2 ).toEqual( event.data );
            } )
            .then( () => {
               expect( data1 === event.data ).toBe( false );
               expect( data2 === event.data ).toBe( false );
