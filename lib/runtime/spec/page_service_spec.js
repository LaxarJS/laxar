...
describe( 'A page service instance', () => {
   ...
   beforeEach( () => {
      debugEventBusMock = createEventBusMock( { nextTick: f => { window.setTimeout( f, 0 ); } } );
   ...
   describe( 'provides a page controller that', () => {
      ...
      const TEST_INSTANCE = { instance: 'test-id0', flow: 'main', id: 'main-id0' };
      let pageElement;
      let pageController;
      ...
      beforeEach( () => {
         ...
         pageController = pageService.createControllerFor( pageElement, TEST_INSTANCE );
      } );
      ...
      describe( 'when asked to setup a page', () => {
         ...
         it( 'requests loading of all widgets', () => {
            expect( widgetLoaderMock.load )
               .toHaveBeenCalledWith(
                  { area: 'testArea1', id: 'id1', widget: 'someWidgetRef1' },
                  jasmine.any( Object ),
                  TEST_INSTANCE );
            expect( widgetLoaderMock.load )
               .toHaveBeenCalledWith(
                  { area: 'testArea2', id: 'id3', widget: 'someWidgetRef2' },
                  jasmine.any( Object ),
                  TEST_INSTANCE );
            expect( widgetLoaderMock.load )
               .toHaveBeenCalledWith(
                  { area: 'nestedLayout.content', id: 'id4', widget: 'someWidgetRef3' },
                  jasmine.any( Object ),
                  TEST_INSTANCE );
         } );
         ...
         it( 'renders the layout for the page', () => {
            ...
            expect( popupsArea.getAttribute( 'ax-widget-area' ) ).toEqual( 'axPopups' );
         } );
         ...
         it( 'automatically adds only missing default widget areas', () => {
            ...
            expect( activitiesArea.getAttribute( 'data-ax-widget-area' ) ).toEqual( 'axActivities' );
            expect( activitiesArea.style.display ).toEqual( 'none' );
            expect( popoversArea.nodeName.toLowerCase() ).toEqual( 'div' );
            expect( popoversArea.getAttribute( 'data-ax-widget-area' ) ).toEqual( 'axPopovers' );
            expect( empty ).not.toBeDefined();

            expect( areaHelper.register )
               .toHaveBeenCalledWith( 'axActivities', activitiesArea, 'axActivities' );
            expect( areaHelper.register )
               .toHaveBeenCalledWith( 'axPopovers', popoversArea, 'axPopovers' );
         } );
