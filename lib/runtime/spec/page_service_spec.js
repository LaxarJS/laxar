...
describe( 'A page service instance', () => {
   ...
   beforeEach( () => {
      ...
      pageLoaderMock.load.and.returnValue( q.when( pageData.page ) );

      layoutLoaderMock = jasmine.createSpyObj( 'layoutLoader', [ 'load' ] );
      layoutLoaderMock.load.and.returnValue( q.when( pageData.layout ) );
      ...
      widgetLoaderMock.load.and.callFake( conf => {
         ...
         return q.when( createdAdapterWrappers[ createdAdapterWrappers.length - 1 ] );
      } );
