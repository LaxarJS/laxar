...
describe( 'A flow service instance', () => {
   ...
   beforeEach( () => {
      ...
      configurationData = createMockConfigurationData();
      configurationMock = createConfigurationMock( configurationData );

      pageRouterMock = createPageRouterMock( browserMock, eventBusMock, createMockRoutes() );
      pageControllerMock = {
   ...
   describe( 'using hashbang URLs, at a given place', () => {
      ...
      it( 'calculates absolute URLs from target and parameters, using a hashbang fragment', () => {
         expect( flowService.absoluteUrl( 'next', { dataId: 42 } ) )
            .toEqual( '/path#!/editor/42' );
      } );

      ////////////////////////////////////////////////////////////////////////////////////////////////////////

      it( 'calculates absolute URLs from place name and parameters, using a hashbang fragment', () => {
         expect( flowService.absoluteUrl( 'editor', { dataId: 42 } ) )
            .toEqual( '/path#!/editor/42' );
      } );
      ...
      it( 'calculates absolute URLs from place name without parameters, using a hashbang fragment', () => {
         expect( flowService.absoluteUrl( 'editor', {} ) )
            .toEqual( '/path#!/editor/_' );
      } );

      ////////////////////////////////////////////////////////////////////////////////////////////////////////

      it( 'escapes parameters when calculating a hashbang fragment', () => {
         expect( flowService.absoluteUrl( 'editor', { dataId: 'evil/manipulation' } ) )
            .toEqual( '/path#!/editor/evil%2Fmanipulation' );
         expect( flowService.absoluteUrl( 'editor', { dataId: 'nefarious hackery' } ) )
            .toEqual( '/path#!/editor/nefarious%20hackery' );
         expect( flowService.absoluteUrl( 'editor', { dataId: 'insiduous?tampering=true' } ) )
            .toEqual( '/path#!/editor/insiduous%3Ftampering%3Dtrue' );
      } );
   ...
   describe( 'using pushState/HTML5 URLs, without explicit base', () => {
      ...
      beforeEach( done => {
         ...
         browserMock.resolve.and.callFake( url => {
            expect( url ).toEqual( '.' );
            return 'https://otherserver:9101/other/path';
         } );
         flowService = createFlowService();
         flowService.controller().loadFlow()
            .then( () => pageRouterMock.initialNavigationPromise )
   ...
   it( 'offers constructAbsoluteUrl as an alias to absoluteUrl', () => {
      flowService = createFlowService();
      expect( flowService.constructAbsoluteUrl ).toBe( flowService.absoluteUrl );
   } );
   ...
   describe( 'with completely set-up flow controller', () => {
      ...
      describe( 'on navigate request to a different target omitting parameters', () => {
         ...
         it( 'uses previous values where available', () => {
