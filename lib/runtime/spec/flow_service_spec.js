...
describe( 'A flow service instance', () => {
   ...
   beforeEach( () => {
      ...
      pageControllerMock = {
         tearDownPage: jasmine.createSpy( 'tearDownPage' ).and.callFake( () => q.when() ),
         setupPage: jasmine.createSpy( 'setupPage' ).and.callFake( () => q.when() ),
      };
      const pageServiceMock = { controller: () => pageControllerMock };
      flowService =
         flowServiceModule.create( frp, eventBusMock, conf, browser, pageServiceMock, pageRouterMock );
   ...
   describe( 'provides a flow controller', () => {
      ...
      it( 'that validates the flow data to load', done => {
         flowController.loadFlow( INVALID_FLOW_JSON )
            .then( done.fail, err => {
               ...
               expect( log.error ).toHaveBeenCalledWith( 'Missing required property: places. Path: "$.places".' );
               expect( log.error ).toHaveBeenCalledWith( 'Additional properties not allowed: its. Path: "$.its".' );
            } )
            .then( done );
      } );
      ...
      describe( 'when the flow is loaded', () => {
         ...
         it( 'allows to return the place for a given navigation target or place', () => {
            expect( flowController.placeNameForNavigationTarget( 'next', welcomePlace ) ).toEqual( 'editor' );
            expect( flowController.placeNameForNavigationTarget( 'home', welcomePlace ) ).toEqual( 'entry' );
            expect( flowController.placeNameForNavigationTarget( '_self', welcomePlace ) ).toEqual( 'welcome' );
            expect( flowController.placeNameForNavigationTarget( 'editor', welcomePlace ) ).toEqual( 'editor' );
            expect( flowController.placeNameForNavigationTarget( 'next', editorPlace ) ).toEqual( 'evaluation' );
         } );
         ...
         it( 'logs the time the navigation took to take place', () => {
            const [ logMessage ] = log.info.calls.mostRecent().args;
            expect( logMessage ).toMatch( /^Timer "navigation \( -> editor\)": start at .*/ );
         } );
