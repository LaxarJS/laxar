...
describe( 'The axPage module', () => {
   ...
   let themeManagerMock_;

   let widgetLoaderMock_;
   let widgetAdapterMocks_;
   let pageServiceMock_;

   beforeEach( () => {

      const configurationMock_ = mockConfiguration();
      ...
      describe( 'that when asked to load a page', () => {
         ...
         describe( 'after setup and attaching widgets', () => {
            ...
            it( 'creates a layout widget adapter for embedded layouts (#193)', () => {
               expect( layoutWidgetAdapter.create )
            ...
            describe( 'and then to tear it down again', () => {
               ...
               let areaVisibilityChangeHandler_;
               let widgetVisibilityChangeHandler_;

               const tearDownCompleteSpy_ = jasmine.createSpy( 'tearDownComplete' );
               const tearDownFailedSpy_ = jasmine.createSpy( 'tearDownFailed' );
   ...
   function mockWidgetAdapter( widgetConfiguration ) {
      const id = widgetConfiguration.id;
      expect( id ).toBeDefined();
      const prefix = 'widgetAdapter.' + id + '.';
      return {
