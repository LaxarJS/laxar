...
import adapterErrors from '../../loaders/widget_loader';
...
beforeEach( () => {
   ...
   widgetFeatures = features.featuresForWidget( descriptor, widgetConfiguration, throwError );
...
describe( 'A plain widget adapter', () => {
   ...
   describe( 'asked to instantiate a widget', () => {
      ...
      it( 'calls onBeforeControllerCreation with injected services', () => {
         expect( environment.onBeforeControllerCreation ).toHaveBeenCalled();

         const [ services ] = environment.onBeforeControllerCreation.calls.argsFor( 0 );
