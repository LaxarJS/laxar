...
describe( 'storage', () => {
   ...
   it( 'getItem, setItem and removeItem calls will be redirected to the provided storage backends', () => {
      ...
      expect( fakeLocalStorage.setItem ).toHaveBeenCalledWith( 'ax.0.x.A', '"4711"' );

      storage.getLocalStorage( 'y' ).getItem( 'B' );
      expect( fakeLocalStorage.getItem ).toHaveBeenCalledWith( 'ax.0.y.B' );

      storage.getLocalStorage( 'z' ).removeItem( 'C' );
      expect( fakeLocalStorage.removeItem ).toHaveBeenCalledWith( 'ax.0.z.C' );
   ...
   describe( 'with no configured storage prefix but a configured application name', () => {
      ...
      it( 'uses a hash of the name to namespace keys', () => {
         ...
         storage.getLocalStorage( 'x' ).setItem( 'A', '4711' );
         expect( fakeLocalStorage.setItem ).toHaveBeenCalledWith( 'ax.444424363.x.A', '"4711"' );

         storage.getLocalStorage( 'y' ).getItem( 'B' );
         expect( fakeLocalStorage.getItem ).toHaveBeenCalledWith( 'ax.444424363.y.B' );
