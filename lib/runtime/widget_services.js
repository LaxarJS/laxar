...
export function create(
   ...
   function createEventBusForWidget( eventBus, widgetName, widgetId ) {
      ...
      const subscriptions = [];
      const unsubscribe = subscriber => { eventBus.unsubscribe( subscriber ); };

      return {
         addInspector( inspector ) {
            const removeInspector = eventBus.addInspector( inspector );
            inspectorRemoveFunctions.push( removeInspector );
            return () => {
               removeInspector();
               const index = inspectorRemoveFunctions.indexOf( removeInspector );
               if( index !== -1 ) {
                  inspectorRemoveFunctions.splice( index, 1 );
               }
            };
         },
         unsubscribe,
         ...
         release() {
            ...
            inspectorRemoveFunctions.forEach( _ => { _(); } );
            inspectorRemoveFunctions.length = 0;
         }
      };
   }
