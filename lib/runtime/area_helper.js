...
export function findWidgetAreas( rootElement ) {
   ...
   //       areas[ name ] = elem;
   //    } );
   const areasMatches = rootElement.innerHTML.match( /(ax-widget-area="\w+")/g );
   if( areasMatches === null ) { return areas; }
   areasMatches.forEach( area => {
      const name = area.match( /"(\w+)"/ )[1];
      if( name !== null ) {
         areas[ name ] = {
            outerHTML: `<div ${area}></div>`,
            innerHTML: '',
            children: [],
            appendChild: ( el ) => {
               areas[ name ].children.push( el );
            }
