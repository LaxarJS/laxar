...
export function debounce( f, waitMs, immediate ) {
   ...
   const debounced = function( ...args ) {
      ...
      timestamp = tooling.nowMilliseconds();
      const callNow = immediate && !timeout;

      if( !timeout ) {
         timeout = tooling.setTimeout( later, waitMs );
      }
      if( callNow && !canceled ) {
         result = f.apply( context, originalArgs );
      }
      ...
      function later( _force ) {
         const sinceLast = tooling.nowMilliseconds() - timestamp;
         if( _force || sinceLast >= waitMs ) {
            timeout = null;
            if( !immediate && !canceled ) {
               result = f.apply( context, originalArgs );
               if( !timeout ) {
                  context = originalArgs = null;
               }
            }
            return;
         }
         timeout = tooling.setTimeout( later, waitMs - sinceLast );
      }
   };
