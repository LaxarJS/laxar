...
function getOrFakeBackend( webStorageName ) {
   ...
   if( window.console ) {
      const method = 'warn' in window.console ? 'warn' : 'log';
 ...
 */
export function create( localStorageBackend, sessionStorageBackend ) {

   const localBackend = localStorageBackend || getOrFakeBackend( LOCAL );
   const sessionBackend = sessionStorageBackend || getOrFakeBackend( SESSION );
   const prefix = 'ax.' + generateUniquePrefix() + '.';
