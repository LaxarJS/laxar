...
function Assertion( subject, optionalDetails ) {
   ...
   this.details = optionalDetails || null;
}
...
Assertion.prototype.isNotNull = function isNotNull( optionalDetails ) {
   if( typeof this.subject === 'undefined' || this.subject === null ) {
      fail( 'Expected value to be defined and not null.', optionalDetails || this.details );
   }
...
Assertion.prototype.hasType = function hasType( type, optionalDetails ) {
   ...
   if( !checkType( this.subject, type ) ) {
      const actualString = functionName( this.subject.constructor );
      const expectedString = functionName( type );

      fail( `Expected value to be an instance of "${expectedString}" but was "${actualString}".`,
            optionalDetails || this.details );
   }
...
Assertion.prototype.hasProperty = function hasProperty( property, optionalDetails ) {
   if( typeof this.subject !== 'object' ) {
      fail( `value must be an object. Got ${typeof this.subject}.` );
   }

   if( !( property in this.subject ) ) {
      fail( `value is missing mandatory property "${property}".`, optionalDetails || this.details );
   }
