...
export function join( ...fragments ) {
   ...
   const segments = fragments.reduce( ( fragments, fragment ) => {
      ...
      if( matchAbsolute ) {
         ...
         return matchAbsolute[ 2 ].split( PATH_SEPARATOR );
      }

      return fragments.concat( fragment.split( PATH_SEPARATOR ) );
   }, [] );

   const pathStack = normalizeFragments( segments );
...
export function normalize( path ) {
   ...
   if( matchAbsolute ) {
      prefix = matchAbsolute[ 1 ];
      remainder = matchAbsolute[ 2 ];
   }

   const pathStack = normalizeFragments( remainder.split( PATH_SEPARATOR ) );
   return prefix + pathStack.join( PATH_SEPARATOR );
}
...
export function relative( from, path ) {
   ...
   return fromStack.reduce( ( path, fragment ) => {
      if( path[ 0 ] === fragment ) {
         path.shift();
      }
      else {
...
const schemeLoaders = {
   local( ref, defaultAssetDirectory ) {
...
function normalizeFragments( fragments ) {
   return fragments.reduce( ( pathStack, rawFragment ) => {
