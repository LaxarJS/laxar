...
function Logger( configuration, channels = [] ) {
   ...
   this.levelToName = ( ( logger, levels ) => {
      const result = {};
      forEach( levels, ( level, levelName ) => {
         logger[ levelName.toLowerCase() ] = ( ...args ) => self.log.apply( this, [ level, ...args ] );
...
Logger.prototype.addLogChannel = function( channel ) {
   ...
   this.messageQueue.forEach( function( entry ) {
      channel( entry );
   } );
};
...
Logger.prototype.gatherTags = function() {
   const tags = {};
   forEach( this.tags, function( values, tag ) {
...
Logger.prototype.setLogThreshold = function( threshold ) {
   if( typeof threshold === 'string' ) {
      ...
      this.threshold = this.level[ threshold.toUpperCase() ];
   }
   else {
...
function gatherSourceInformation() {
   ...
   if( rows[ 0 ] === 'Error' ) {
      ...
      interpreterFunction = function fireFoxStackInterpreter( row ) {
         const match = FIRE_FOX_STACK_MATCHER.exec( row );
         return {
            file: match ? match[ 1 ] : '?',
            line: match ? match[ 2 ] : -1,
            char: -1
   ...
   for( let i = 0; i < rows.length; ++i ) {
      const row = interpreterFunction( rows[ i ] );
