...
import { create, level } from '../log';

describe( 'A logger', () => {

   let configurationMock_;
   let configurationValues_;
   let logger_;
   let spyChannel;

   beforeEach( () => {
      configurationValues_ = { 'logging.level': 'INFO' };
      spyChannel = jasmine.createSpy( 'spyChannel' );
      configurationMock_ = createConfigurationMock( configurationValues_ );
      logger_ = create( configurationMock_ );
   } );
   ...
   describe( 'defines shortcut log methods', () => {
      ...
      Object.keys( level ).forEach( levelName => {
   ...
   describe( 'configured with custom levels', () => {
      ...
      beforeEach( () => {
         configurationValues_[ 'logging.levels' ] = { NOTICE: 350, HELL: 666 };
         logger_ = create( configurationMock_ );
         logger_.addLogChannel( spyChannel );
      } );
