...
import { create, level, BLACKBOX } from '../log';
...
describe( 'A logger', () => {
   ...
   beforeEach( () => {
      configurationValues = { 'logging.level': 'INFO' };
   ...
   describe( 'defines shortcut log methods', () => {
      ...
      Object.keys( level ).forEach( levelName => {
         ...
         it( `for log level ${levelName}`, () => {
            ...
            expect( logger.log ).toHaveBeenCalledWith( logger.level[ levelName ], 'message', BLACKBOX );
         } );
   ...
   describe( 'supports to set a threshold', () => {
      ...
      beforeEach( () => {
         ...
         logger.addLogChannel( spyChannel );
         logger.setLogThreshold( logger.level.WARN );
      } );
