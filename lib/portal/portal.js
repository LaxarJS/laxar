...
define( [
   ...
   './paths'
], function( angular, $, path, languageShim, paths ) {
   'use strict';

   var module = angular.module( 'laxar.portal', [] );
   ...
   module.run( [ 'CssLoader', 'ThemeManager', function( CssLoader, themeManager ) {
      // NEEDS FIX A: Must be removed from here, but where should this take place instead?
      // serviceInterface.init( $http, $q, $timeout );
      // serviceInterface.setLogTag( 'INST', storage.getApplicationInstanceId() );
      // serviceInterface.setGlobalRepositoryUrls( Configuration.get( 'serviceInterface.repositoryUrls' ) );
   ...
   // NEEDS FIX A: Shim for the deprecated language module. Must be removed!
   module.run( [
      '$q', 'Configuration', 'FileResourceProvider', 'EventBus',

      function( $q, Configuration, fileResourceProvider, eventBus ) {
         languageShim.init(
            $q,
            fileResourceProvider,
            eventBus,
            Configuration.get( 'locales', { 'default': 'en_US' } )[ 'default' ].replace( '-', '_' )
         );
      }
   ] );
