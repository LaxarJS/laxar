...
export function create( debugEventBus ) {
   ...
   const api = {
      ...
      registerDebugInfo,
      registerItem( item ) {
         //addLoadingStateHandler( 'flow', item, instanceState );
         addLoadingStateHandler( 'page', item, instanceState );
         //addLoadingStateHandler( 'widget', item, instanceState );
      }
   };
   ...
   function addLoadingStateHandler( type, { instance, id }, states ) {
      const object = states[ id ] = ( states[ id ] || {} );
      const array = object[ type ] = [];

      debugEventBus.subscribe( `didLoad.${type}.${instance}.${id}`, ( { [ type ]: ref } ) => {
         array.push( ref );
      } );
      debugEventBus.subscribe( `didUnload.${type}.${instance}.${id}`, ( { [ type ]: ref } ) => {
