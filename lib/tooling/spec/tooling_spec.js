...
define( [
   ...
   describe( 'The page tooling API', function() {
      ...
      beforeEach( function() {
         pageTooling.setPageDefinition( 'p', { name: 'my page' } );
         pageTooling.setPageDefinition( 'x', { name: 'x' } );
      ...
      it( 'allows to access page definitions by reference', function() {
         expect( pageTooling.current().pageDefinitions.p ).toEqual( { name: 'my page' } );
            expect( pageTooling.current().pageDefinitions.x ).toEqual( { name: 'x' } );
      } );
