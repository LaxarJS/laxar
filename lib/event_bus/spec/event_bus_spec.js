...
define( [
   ...
   describe( 'Errors when calling subscribers', function() {
      ...
      describe( 'providing a set of useful information that', function() {
         ...
         it( 'contains the meta information', function() {
   ...
   describe( 'An event bus can have a mediator that', function() {
      ...
      it( 'is called before calling subscribers with the current set of queued event items in order they were published', function() {
         ...
         expect( mediatorSpy.calls[0].args[0][0].meta.name ).toEqual( 'myEvent.1' );
         expect( mediatorSpy.calls[0].args[0][1].meta.name ).toEqual( 'myEvent.2' );
         expect( mediatorSpy.calls[0].args[0][2].meta.name ).toEqual( 'myEvent.3' );
      } );
      ...
      it( 'can remove event items', function() {
         ...
         eventBus.setMediator( function( eventItems ) {
            ...
            for( var i = 0; i < eventItems.length; ++i ) {
               if( eventItems[ i ].meta.name !== 'myEvent.2' ) {
   ...
   describe( 'an event bus can have inspectors', function() {
      ...
      it( 'that are called for every new subscription', function() {
      ...
      it( 'that are called for every publish', function() {
      ...
      it( 'that are called every time an event is delivered to a subscriber', function() {
      ...
      it( 'that are called on every unsubscribe (#20)', function() {
      ...
      it( 'that can be removed via its remove function', function() {
         removeFunction();
   ...
   describe( 'An event object', function() {
      ...
      describe( 'where the subscriber requests cloning', function() {
      ...
      describe( 'where the subscriber doesn\'t request a clone', function() {
         ...
         if( Object.freeze && Object.isFrozen ) {
            it( 'gets a frozen event in a browser supporting Object.freeze', function() {
               eventBus.publish( 'myEvent', { data: { x: 12  } } );
               jasmine.Clock.tick( 0 );
            ...
            it( 'gets the original event in a browser not supporting Object.freeze', function() {
               eventBus.publish( 'myEvent', { data: { x: 12  } } );
               jasmine.Clock.tick( 0 );
   ...
   describe( 'A meta object', function() {
      ...
      describe( 'has a cycleId', function() {
         ...
         it( 'that is counted up on each publish', function() {
            ...
            expect( mySpy.calls[ 0 ].args[ 1 ].cycleId + 2 ).toEqual( mySpy.calls[ 1 ].args[ 1 ].cycleId );
         } );
         ...
         describe( 'when triggered during a delivery', function() {
            ...
            it( 'it must have the same cycle id as the event it was delivered on', function() {
               expect( mySpy2.calls[ 0 ].args[ 1 ].cycleId ).toBeDefined();
               expect( mySpy2.calls[ 0 ].args[ 1 ].cycleId ).toEqual( cycleId );
            } );
   ...
   describe( 'and when multiple will-responses are completed with a single did', function() {
      ...
      beforeEach( function() {
         ...
         eventBus = eventBusModule.create( { pendingDidTimeout: 100 } );
         eventBus.setErrorHandler( errorHandlerSpy );

         numRequests = 0;
         numReplies = 0;
         eventBus.subscribe( 'workAsyncRequest', function() {
            eventBus.publish( 'willWorkAsync', {}, { sender: 'asyncSender' } );
            if( numRequests === 0 ) {
               window.setTimeout( function() {
                  eventBus.publish( 'didWorkAsync', {}, { sender: 'asyncSender' } );
                  jasmine.Clock.tick( 0 );
               }, 10 );
            }
            ++numRequests;
         ...
         eventBus.publishAndGatherReplies( 'workAsyncRequest' )
            .then( function() { ++numReplies; }, rejectSpy );
         eventBus.publishAndGatherReplies( 'workAsyncRequest' )
            .then( function() { ++numReplies; }, rejectSpy );
      ...
      it( 'does not run into a timeout', function() {
   ...
   describe( 'An event bus supports a request-will-did pattern', function() {
      ...
      it( 'whose promise is resolved only after all will-did replies were given asynchronously', function() {
         ...
         expect( mySpy.calls[0].args[0][0].meta.name ).toEqual( 'didDoSomethingAsync.andItWorked' );
      } );
      ...
      it( 'whose promise is resolved only after all will-did replies were given synchronously', function() {
         ...
         expect( mySpy ).toHaveBeenCalled();
         expect( mySpy.calls[0].args[0].length ).toEqual( 1 );
         expect( mySpy.calls[0].args[0][0].meta.name ).toEqual( 'didDoSomethingSync.andItWorked' );
      } );
      ...
      it( 'only listens to did/will answers for the given subject', function() {
         ...
         expect( mySpy ).toHaveBeenCalled();
         expect( mySpy.calls[0].args[0].length ).toEqual( 2 );
         expect( mySpy.calls[0].args[0][0].meta.name ).toEqual( 'didDoSomethingSync.subTopic.andItWorked' );
         expect( mySpy.calls[0].args[0][1].meta.name ).toEqual( 'didDoSomethingSync.subTopic.andItDidntWork' );
      } );

      ////////////////////////////////////////////////////////////////////////////////////////////////////////

      it( 'works with topics that contain the string "Request"', function() {
         eventBus.subscribe( 'validateRequest.exportRequest', function() {
         ...
         eventBus.publishAndGatherReplies( 'validateRequest.exportRequest' ).then( mySpy );

         jasmine.Clock.tick( 0 );

         expect( mySpy ).toHaveBeenCalled();
         expect( mySpy.calls[0].args[0].length ).toEqual( 1 );
         expect( mySpy.calls[0].args[0][0].meta.name ).toEqual( 'didValidate.exportRequest' );
      } );
      ...
      describe( 'when a collaborator does not send a did after will within a given timeout', function() {
         ...
         beforeEach( function() {
            ...
            eventBus.publishAndGatherReplies( 'doSomethingWithTimeoutRequest' ).then( mySpy, rejectSpy );
            jasmine.Clock.tick( 11 );
         } );

         /////////////////////////////////////////////////////////////////////////////////////////////////////

         it( 'the promise is rejected with all results available up to the timeout', function() {
            expect( mySpy ).not.toHaveBeenCalled();
            expect( rejectSpy ).not.toHaveBeenCalled();
            ...
            expect( rejectSpy.calls[0].args[0].length ).toEqual( 1 );
            expect( rejectSpy.calls[0].args[0][0].meta.sender ).toEqual( 'sender1' );
         } );
