 ...
 */
import { init as initEventBus, create as createEventBus } from '../event_bus';
import * as q from 'q';
...
beforeEach( () => {
   ...
   initEventBus( q, nextTick, setTimeout );
...
describe( 'Errors when calling subscribers', () => {
   ...
   describe( 'providing a set of useful information that', () => {
...
describe( 'An event bus can have a mediator that', () => {
   ...
   it( 'can remove event items', done => {
      ...
      // NEEDS FIX B: deferred of this event will never be resolved, since the mediator removes it from
