 ...
 * @private
 */
function EventBus( nextTick, timeoutFunction, browser, optionalConfiguration ) {
   this.nextTick = f => nextTick( f );
   this.timeoutFunction = ( f, ms ) => timeoutFunction( f, ms );
   this.config = object.options( optionalConfiguration, {
   ...
   this.waitingPromiseResolves = [];
   this.currentCycle = -1;
   this.errorHandler = createDefaultErrorHandler( browser );
   this.mediator = ensureFunction();
   this.inspectors = [];
}
...
EventBus.prototype.setErrorHandler = function( errorHandler ) {
   this.errorHandler = ensureFunction( errorHandler, this.errorHandler );
};
...
EventBus.prototype.setMediator = function( mediator ) {
   this.mediator = ensureFunction( mediator );
};
...
EventBus.prototype.unsubscribe = function( subscriber ) {
   ...
   const self = this;
   const subscriberTree = this.subscriberTree;
...
EventBus.prototype.publish = function( eventName, optionalEvent, optionalOptions ) {
   ...
   return new Promise( resolve => {
      const eventItem = {
         meta: {
            name: eventName,
            cycleId: this.currentCycle > -1 ? this.currentCycle : this.cycleCounter++,
...
EventBus.prototype.publishAndGatherReplies = function( eventName, optionalEvent, optionalOptions ) {
   ...
   const self = this;
   const options = object.options( optionalOptions, {
      pendingDidTimeout: this.config.pendingDidTimeout
   } );
   ...
   const timeoutRef = this.timeoutFunction( function() {
...
function enqueueEvent( self, eventItem ) {
   if( self.eventQueue.length === 0 ) {
      self.nextTick( function() {
         const queuedEvents = self.eventQueue;
   ...
   self.eventQueue.push( eventItem );
}

//////////////////////////////////////////////////////////////////////////////////////////////////////////////

function processQueue( self, queuedEvents ) {
   return self.mediator( queuedEvents ).map( function( eventItem ) {
      const meta = eventItem.meta;
      self.currentCycle = meta.cycleId;
...
function processWaitingPublishPromises( self, newPromiseResolves ) {
   const waitingResolves = self.waitingPromiseResolves;
   self.waitingPromiseResolves = newPromiseResolves;
...
function findSubscribers( self, eventName ) {
   const subscribers = [];
   const parts = eventName.split( PART_SEPARATOR );
   const node = self.subscriberTree;
...
function notifyInspectors( self, infoObject ) {
   self.inspectors.forEach( function( inspector ) {
...
export function createDefaultErrorHandler( browser ) {
   return ( message, optionalErrorInformation ) => {
      const browserConsole = browser.console();
      if( !browserConsole || !browserConsole.log ) {
         return;
      }

      const errFunc = browserConsole.error ? 'error' : 'log';
      browserConsole[ errFunc ]( message );
      ...
      if( optionalErrorInformation ) {
         Object.keys( optionalErrorInformation ).forEach( function( title ) {
            ...
            browserConsole[ errFunc ]( '   - %s: %o', title, info );
            if( info instanceof Error && info.stack ) {
               browserConsole[ errFunc ]( '   - Stacktrace: %s', info.stack );
            }
         } );
      }
   };
...
export function createLogErrorHandler( log ) {
   return ( message, optionalErrorInformation ) => {
...
function ensureFunction( candidate, fallback ) {
   return typeof candidate === 'function' ? candidate : ( fallback || ( _ => _ ) );
}
 ...
 * @return {EventBus}
 */
export function create( nextTick, timeoutFunction, browser, optionalConfiguration ) {
   assert( nextTick )
   ...
   assert( browser.console ).hasType( Function ).isNotNull();

   return new EventBus( nextTick, timeoutFunction, browser, optionalConfiguration );
}
