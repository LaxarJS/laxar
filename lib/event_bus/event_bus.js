...
function EventBus( configuration, log, nextTick, timeoutFunction ) {
   ...
   this.timeoutMs_ = configuration.get( 'eventBusTimeoutMs', 120000 );
   ...
   this.errorHandler_ = createLogErrorHandler( log );
   this.inspectors_ = [];
   this.log_ = log;
}
...
EventBus.prototype.unsubscribe = function( subscriber ) {
   ...
   this.log_.warn(
      'Deprecation: EventBus.unsubscribe() is deprecated and will be removed in a future release. ' +
      'Use the unsubscribe function returned by `subscribe` instead'
   );
...
function unsubscribe( self, subscriber ) {
   if( !Array.isArray( subscriber[ INTERNAL_EVENTS_REGISTRY ] ) ) {
      return;
   }

   subscriber[ INTERNAL_EVENTS_REGISTRY ].forEach( eventName => {
      unsubscribeRecursively( self, self.subscriberTree_, eventName.split( PART_SEPARATOR ), subscriber );
   } );
      ...
      subscribers.forEach( subscriberItem => {
         ...
         try {
            ...
            subscriberItem.subscriber( event, object.options( meta, {
               unsubscribe() {
                  self.log_.warn(
                     'Deprecation: unsubscribe() is deprecated and will be removed in a future release. ' +
                     'Use the unsubscribe function returned by `subscribe` instead'
                  );
                  unsubscribe( self, subscriberItem.subscriber );
               }
            } ) );
         }
         catch( e ) {
 ...
 */
export function create( configuration, log, nextTick, timeoutFunction ) {
   assert( configuration.get ).hasType( Function ).isNotNull();
   ...
   return new EventBus( configuration, log, nextTick, timeoutFunction );
}
