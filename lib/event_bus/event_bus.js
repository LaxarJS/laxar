 ...
 * @private
 */
function EventBus( q, nextTick, timeoutFunction, optionalConfiguration ) {
   this.q_ = q;
...
EventBus.prototype.publish = function( eventName, optionalEvent, optionalOptions ) {
   ...
   const eventItem = {
      ...
      event: event,
      publishedDeferred: this.q_.defer()
   };
...
function processWaitingDeferreds( self, newDeferreds ) {
   const waitingDeferreds = self.waitingDeferreds_;
 ...
 * @return {EventBus}
 */
export function create( q, nextTick, timeoutFunction, optionalConfiguration ) {
   ...
   return new EventBus( q, nextTick, timeoutFunction, optionalConfiguration );
}
