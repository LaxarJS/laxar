...
define( [
   ...
   return {
       ...
       *
       * @deprecated use ax.string.format instead
       */
       ...
       * @deprecated use ax.string.format instead
       */
      format: function textFormatToStringSubstitute( str, vargs ) {
         var indexed = [];
         var named = {};
         [].slice.call( arguments, 1 ).forEach( function( arg, index ) {
            if( arg.name && arg.value && arg.getFormatter ) {
         ...
         return stringUtils.format( str, indexed, named );
      },
