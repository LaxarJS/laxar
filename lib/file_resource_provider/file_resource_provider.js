 ...
 * @private
 */
function FileResourceProvider( q, httpClient, rootPath ) {
   this.q_ = q;
...
FileResourceProvider.prototype.provide = function( url ) {
   ...
   return entry( this, url ).then( function( knownEntry ) {
      if( typeof( knownEntry ) === 'string' ) {
         return self.q_.when( knownEntry ).then( resourceTransform( url ) );
      }
      return knownEntry !== false ? httpGet( self, url ).then( resourceTransform( url ) ) : self.q_.reject();
   }, function() {
...
FileResourceProvider.prototype.isAvailable = function isAvailable( url ) {
   ...
   return entry( self, url ).then( function( knownEntry ) {
      return self.q_.when( knownEntry !== false );
   }, function() {
      return httpHead( self, url ).then( function( knownAvailable ) {
         return self.q_.when( knownAvailable );
      } );
  } );
};
...
function entry( self, resourcePath ) {
   ...
   if( usablePrefixes.length ) {
      ...
      return fetchListingForPath( self, prefix ).then( function( listing ) {
         return self.q_.when( lookup( self, resourcePath, listing ) );
      } );
   }
...
function fetchListingForPath( self, path ) {
   if( self.fileListings_[ path ] ) {
      return self.q_.when( self.fileListings_[ path ] );
   }
...
function httpGet( self, url ) {
   ...
   const promise = self.httpGets_[ url ] = self.httpClient_
      .get( url, { transformResponse: [] } )
      .then( function( response ) {
         return self.q_.when( response.data );
      } );
...
function httpHead( self, url ) {
   if( url in self.httpHeadCache_ ) {
      return self.q_.when( self.httpHeadCache_[ url ] );
   }
   if( url in self.httpHeads_ ) {
      return self.httpHeads_[ url ];
   }

   const promise = self.httpHeads_[ url ] = self.httpClient_.head( url )
      .then( function() {
         return true;
      }, function() {
         return false;
 ...
 *    a new instance
 */
export function create( q, httpClient, rootPath ) {
   assert( q ).isNotNull( 'Need a promise implementation like $q or Q' );
   assert( httpClient ).isNotNull( 'Need a http client implementation like $http' );

   return new FileResourceProvider( q, httpClient, rootPath );
}
