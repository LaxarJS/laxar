...
function entry( self, resourcePath ) {
   const usablePrefixes = Object.keys( self.fileListingUris_ )
      .filter( prefix => resourcePath.indexOf( prefix ) === 0 );

   if( usablePrefixes.length ) {
      const prefix = usablePrefixes[ 0 ];
      return fetchListingForPath( self, prefix )
         .then( listing => Promise.resolve( lookup( self, resourcePath, listing ) ) );
   }
