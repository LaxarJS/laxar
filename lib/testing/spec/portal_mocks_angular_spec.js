...
define( [
   ...
   describe( 'A ControllerTestBed with custom injections', function() {
      ...
      it( 'supports descriptor-based invocation (LaxarJS/laxar#227)', function() {
         createAngularModule( null, controllerSpy, [ '$scope' ], {
            moduleName: 'myAwesomeWidget',
            controllerName: 'MyAwesomeWidgetController'
         } );
         testBed = portalMocksAngular.createControllerTestBed( { name: 'my-awesome-widget' } );
         spyOn( testBed.$, 'ajax' );
         testBed.setup();
         expect( controllerSpy ).toHaveBeenCalledWith( testBed.scope );

         // Make sure that the request for the widget.json was intercepted
         testBed.$.ajax.calls.forEach( function( call ) {
            var url = call.args[ 0 ].url;
            expect( url ).not.toEqual( '/my-awesome-widget/widget.json' );
         } );
      } );
      ...
      it( 'provides a mocked axControls service as an injection', function() {
         createAngularModule( null, controllerSpy, [ 'axControls' ] );
         testBed = portalMocksAngular.createControllerTestBed( 'my-awesome-widget' );
         mockAjax( 'my-awesome-widget', testBed );
         testBed.setup();
         expect( controllerSpy ).toHaveBeenCalled();
         var injectedService = controllerSpy.calls[ 0 ].args[ 0 ];
         expect( injectedService.provide ).toEqual( jasmine.any( Function ) );
         expect( injectedService.resolve ).toEqual( jasmine.any( Function ) );
         expect( injectedService.descriptor ).toEqual( jasmine.any( Function ) );
         expect( injectedService.descriptor ).toEqual( jasmine.any( Function ) );
      } );
   ...
   describe( 'mockDebounce', function() {
      ...
      it( 'installs an underscore-compatible debounce function that uses the mock clock', function() {
         ...
         expect( fImmediate ).toHaveBeenCalledWith( 'xxx' );
         expect( fImmediate ).not.toHaveBeenCalledWith( 'yyy' );
      ...
      it( 'allows to inspect the list of waiting calls', function() {
         ...
         expect( f ).not.toHaveBeenCalled();
         expect( fImmediate ).toHaveBeenCalledWith( 'xxx' );
         expect( fn.debounce.waiting.length ).toBeGreaterThan( 0 );
