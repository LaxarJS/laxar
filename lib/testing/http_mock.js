...
HttpMock.prototype.createResponse_= function( method, uri, transform ) {
   ...
   const responses = this.responseMap[ method ];
   const response  = responses ? deepClone( responses[ uri ] ) : null;
...
[ 'del' ].concat( HTTP_METHODS ).forEach( function( method ) {
   HttpMock.prototype[ method ] = function( uri, dataOrConfig, optionalConfig ) {
      const config = arguments.length === 3 ? optionalConfig : dataOrConfig;

      const promise = this.createResponse_( method.toUpperCase(), uri, ( config || {} ).transformResponse );
...
export function create( optionalQ ) {
   const instance = new HttpMock( optionalQ );

   [ 'del' ].concat( HTTP_METHODS ).forEach( function( method ) {
      instance[ method ] = jasmine.createSpy( method ).and.callFake( function() {
