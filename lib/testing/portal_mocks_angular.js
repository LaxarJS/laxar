...
define( [
    ...
    *    The path that can be used to load this widget into a page (e.g. 'ax-some-widget', 'portal/my_widget')
    *    For backwards-compatibility, an old-style widget module names starting with 'widgets.' may be given instead (See LaxarJS/laxar#153).
   ...
   function createControllerTestBed( widgetPathOrModule, controllerName ) {
      ...
      if( widgetPathOrModule.indexOf( 'widgets.' ) === 0 ) {
         ...
         fullControllerName = string.capitalize( moduleName ) + 'Controller';
      }
   ...
   function getWidgetLoader( testBed ) {
      ...
      return widgetLoaderModule
         .create( q, fileResourceProvider, themeManager, { load: function() {} }, testBed.eventBusMock, {
            adapters: {
   ...
   function portalMocksAngularAdapter( testBed ) {
      ...
      function create( assetResolver, specification, features, widgetConfiguration, anchorElement ) {
         ...
         function createController( widgetServices, configuration ) {
            ...
            var eventBus = testBed.scope.eventBus = widgetServices.eventBus;
            ...
            scope.id = widgetServices.idGenerator;
            scope.widget = {
