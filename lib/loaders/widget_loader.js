...
export function create(
    ...
    * @memberof WidgetLoader
    */
   function load( widgetConfiguration, { whenServicesAvailable = noOp } = {}, itemMeta = {}, pageElement) {
      const widgetArtifactProvider = artifactProvider.forWidget( widgetConfiguration.widget );
      const publishDebugInfo = ( event, data ) => {
         debugEventBus.publish( `${event}.${itemMeta.instance}.${itemMeta.item}`, {
            itemMeta,
            ...data
         } );
      };
      ...
      return widgetArtifactProvider
         ...
         .then( descriptor => {
            ...
            const anchorElement = {
               id: `ax${ID_SEPARATOR}${widgetConfiguration.id}`,
               className: widgetName,
               outerHTML: `<div class="${ anchorElement.className}" id="${anchorElement.id}"></div>`,
               innerHTML: '',
               widgetAreas: {},
               children: [],
               appendChild: ( el ) => {
                  anchorElement.children.push( el );
               }
            };
