...
export function create( configuration, themeManager ) {
   const mergedCssFileLoaded = [].some.call( document.getElementsByTagName( 'link' ), function( link ) {
   ...
   if( configuration.get( 'useMergedCss', false ) ) {
      loader.load( path.join( paths.PRODUCT, 'const/static/css', themeManager.getTheme() + '.theme.css' ) );
      return { load: function() {} };
   }
