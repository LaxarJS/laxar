...
function PageLoader( baseUrl, fileResourceProvider, pageCollector ) {
   ...
   this.idCounter = 0;
   this.pageToolingCollector = pageCollector;
}
...
function loadPageRecursively( self, pageName, extensionChain ) {
   let page;
   const pageUrl = assetUrl( self.baseUrl, pageName );
   ...
   return load( self, pageUrl )
      ...
      .then( () => {
         self.pageToolingCollector.collectPageDefinition( pageName, page, FLAT );
         return page;
      } );
}
   ...
   function processNestedCompositions( page, instanceId, compositionChain ) {
      ...
      object.forEach( page.areas, widgets => {
         widgets.slice().reverse().forEach( widgetSpec => {
            ...
            // up.
            promise = promise
               .then( () => load( self, assetUrl( self.baseUrl, compositionName ) ) )
               ...
               .then( composition => {
                  ...
                  return processNestedCompositions( composition, widgetSpec.id, chain )
                     .then( () => {
                        self.pageToolingCollector.collectCompositionDefinition(
                           topPageName,
                           widgetSpec.id,
                           composition,
                           FLAT
      ...
      if( page === topPage ) {
         self.pageToolingCollector.collectPageDefinition( topPageName, page, COMPACT );
      }
      else {
         self.pageToolingCollector.collectCompositionDefinition( topPageName, instanceId, page, COMPACT );
      }
...
function load( self, url ) {
   return self.fileResourceProvider.provide( url );
}
