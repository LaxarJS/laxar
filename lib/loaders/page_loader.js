...
function PageLoader( q, baseUrl, fileResourceProvider ) {
   this.q_ = q;
 ...
 * @private
 */
export function create( q, baseUrl, fileResourceProvider ) {
   assert( q ).isNotNull();
   assert( baseUrl ).isNotNull();
   assert( fileResourceProvider ).isNotNull();

   return new PageLoader( q, baseUrl, fileResourceProvider );
}
