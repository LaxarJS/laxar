...
define( [
               ...
               if( has( widgetSpec, 'composition' ) ) {
                  ...
                  if( !has( widgetSpec, 'id' ) ) {
                     var escapedCompositionName =
                        widgetSpec.composition.replace( SEGMENTS_MATCHER, capitalized );
   ...
   function prefixCompositionIds( composition, widgetSpec ) {
      composition.areas = object.map( composition.areas, function( widgets, areaName ) {
         ...
         if( areaName.indexOf( '.' ) > 0 ) {
            ...
            return [ widgetSpec.id + ID_SEPARATOR + areaName, widgets ];
         }
      ...
      function replaceExpression( subject ) {
         ...
         if( expression.indexOf( COMPOSITION_TOPIC_PREFIX ) === 0 ) {
            result = topicFromId( widgetSpec.id ) +
               TOPIC_SEPARATOR + expression.substr( COMPOSITION_TOPIC_PREFIX.length );
         }
         else {
         ...
         page.areas[ index ] = widgetList.filter( function( widgetSpec ) {
            ...
            if( has( widgetSpec, 'widget' ) ) {
               if( !has( widgetSpec, 'id' ) ) {
                  ...
                  widgetSpec.id = nextId( self, widgetName.replace( SEGMENTS_MATCHER, capitalized ) );
               }
   ...
   function capitalized( segmentStart ) {
   ...
   function topicFromId( id ) {
      return id.replace( ID_SEPARATOR_MATCHER, TOPIC_SEPARATOR ).replace( SEGMENTS_MATCHER, capitalized );
   }
