...
define( [
   ...
   describe( 'A PageLoader', function() {
      ...
      describe( 'when loading a simple page', function() {
         ...
         it( 'checks that widget ids are unique', function() {
            loadExamplePage( 'pageWithIdConflict' );
            window.foo = rejectedSpy;
         ...
         it( 'detects duplicate widget ids in the same area', function() {
            ...
            expect( rejectedSpy ).toHaveBeenCalledWithError(
               'Error loading page "pageWithDuplicateWidgetIdsInSameArea": Duplicate widget/composition ID(s): id1'
            );
         } );
      ...
      describe( 'when a page uses compositions', function() {
         ...
         it( 'checks widget and composition ids for uniqueness', function() {
            loadExamplePage( 'pageWithIdConflictWidgetVsComposition' );
            expect( rejectedSpy ).toHaveBeenCalled();
         } );
      ...
      describe( 'when loading an invalid page', function() {
         ...
         it( 'rejects the load-promise', function() {
            expect( rejectedSpy ).toHaveBeenCalled();
         } );
      ...
      describe( 'when loading a page with invalid composition feature configureation', function() {

         beforeEach( function() {
            loadExamplePage( 'pageWithFeaturesOfCompositionBadlyConfigured' );
         } );
         ...
         it( 'provides a useful error message', function() {
            expect( rejectedSpy.calls[0].args[0].message ).toEqual(
               ...
               ' - Invalid type: integer should be one of string,null. Path: "$.something.resource".' );
         } );
      ...
      function expectUniqueWidgetIds( page ) {
         ...
         object.forEach( page.areas, function( widgetList ) {
            object.forEach( widgetList, function( widgetSpec ) {
               if( object.hasOwnProperty( widgetSpec, 'widget' ) ) {
                  expect( widgetSpec['id'] ).toBeDefined();
               expect( object.hasOwnProperty( seenIds, widgetSpec['id'] ) ).toBe( false );
            }
