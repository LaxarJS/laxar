...
describe( 'A PageLoader', () => {
   ...
   beforeEach( () => {
      ...
      pageLoader = createPageLoader( baseUrl, frpMock, pagesCollectorMock );
   } );
   ...
   describe( 'when a page uses compositions', () => {
      ...
      it( 'loads simple compositions into its parent area, prefixing ids used in the composition', done => {
         pageLoader.load( 'pageWithSimpleComposition' )
            .then( page => {
               expect( page.areas.area1.length ).toBe( 5 );
               expect( page.areas.area1[ 0 ] ).toEqual( { widget: 'someWidgetPath1', id: 'id1' } );
               expect( page.areas.area1[ 1 ] )
                  .toEqual( { widget: 'laxarjs/test_widget', id: 'simpleComposition-id0-idx1' } );
               expect( page.areas.area1[ 2 ] )
                  .toEqual( { widget: 'laxarjs/test_widget2', id: 'testWidget2-id2' } );
               expect( page.areas.area1[ 3 ] )
                  .toEqual( { widget: 'laxarjs/test_widget2', id: 'testWidget2-id1' } );
               expect( page.areas.area1[ 4 ] )
                  .toEqual( { widget: 'someWidgetPath1', id: 'id2' } );
      ...
      it( 'merges areas existing in the composition and the page', done => {
         pageLoader.load( 'pageWithCompositionWithAdditionalAreas' )
            .then( page => {
               expect( page.areas.area2.length ).toBe( 2 );
               expect( page.areas.area2[ 0 ] ).toEqual( { widget: 'someWidgetPath1', id: 'id3' } );
               expect( page.areas.area2[ 1 ] ).toEqual( {
      ...
      it( 'adds new areas of the composition to the page, prefixing areas in the composition', done => {
         pageLoader.load( 'pageWithCompositionWithAdditionalAreas' )
            .then( page => {
      ...
      it( 'omits widgets that are disabled within compositions (#24)', done => {
         pageLoader.load( 'pageWithCompositionWithDisabledWidgets' )
            .then( page => {
               const { area1, area2 } = page.areas;
               expect( area1.length ).toBe( 3 );
               expect( area1[ 0 ] ).toEqual( { widget: 'someWidgetPath1', id: 'id1' } );
               expect( area1[ 1 ] ).toEqual( { widget: 'laxarjs/test_widget2', id: 'testWidget2-id1' } );
               expect( area1[ 2 ] ).toEqual( { widget: 'someWidgetPath1', id: 'id2' } );

               expect( area2.length ).toBe( 1 );
               expect( area2[ 0 ] ).toEqual( { widget: 'someWidgetPath1', id: 'id3' } );
            } )
            .then( done );
      } );
      ...
      describe( 'when features are missing from the composition configuration (#29)', () => {
         ...
         it( 'removes undefined values in widget features', done => {
      ...
      it( 'keeps null values of widget feature configuration inside of compositions (#28)', done => {
