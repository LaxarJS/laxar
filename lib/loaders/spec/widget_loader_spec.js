...
describe( 'An initialized widgetLoader', () => {
   ...
   beforeEach( () => {
      ...
      http_ = createHttpMock( q );
      controlsServiceMock_ = createControlsServiceMock( mockControls );
      fileResourceProviderMock_ = createFrpMock( fileResources );
      themeManager_ = createThemeManager( fileResourceProviderMock_, q, 'default' );
      cssLoader_ = { load: jasmine.createSpy( 'load' ) };
      eventBusMock_ = createEventBusMock( q );
      ...
      widgetLoader_ = createWidgetLoader( q, {
   ...
   function createControlsServiceMock( initialData ) {
      ...
      return {
         load: jasmine.createSpy( 'axControls.load' ).and.callFake( controlRef => {
            ...
            return q.when( mockControls[ controlRef ].descriptor );
         } ),
